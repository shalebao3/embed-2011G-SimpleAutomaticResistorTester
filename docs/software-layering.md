...
此前以 `cd88988334f7dd3b746ebbbeb622d03bfcead534` 为基线，将 `App / Driver / Bsp / Common / User` 原样收进 `firmware/src/`，将链接脚本原样移入 `firmware/Start/`。目录迁移仅同步构建、校验和文档路径，没有改变已有模块分工。
...
| `firmware/src/Bsp/README.md` | 保留具体硬件模块接口的目录约定，当前没有参与编译的源码，不保留空 LED 驱动 |
...
`CMakeLists.txt`、`CMakePresets.json` 继续放在 firmware 根目录，工具链保留在 `cmake/`，生成文件继续放在已忽略的 `build/`。LED 清理只移除源文件登记和不再需要的 Bsp 头文件路径，不改变工程名、构建预设、编译选项、产物名称或子模块路径和版本。
...
`Driver` 与 `Bsp` 模块不反向调用 `App` 或 `main`；需要时间服务时可以使用 `Common`。App 可以直接调用 ADC 驱动，不创建只转发调用的接口层。后续真正接入继电器时才增加 `Bsp_Range`，不提前堆空模块。
